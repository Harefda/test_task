...
def get_all_answers(actors):
    ...
    for actor in actors:
        ...
        oldest_movie = get_oldest_movie(
            actor, current_year, oldest_movie, oldest_movie_age
        )[0]
        oldest_movie_age = get_oldest_movie(
            actor, current_year, oldest_movie, oldest_movie_age
        )[1]
        actor_with_biggest_filmography = get_actor_with_biggest_filmography(
            actor, actor_with_biggest_filmography
        )
        count_films = get_count_fimls(actor, count_films)
        actor_dead_for_the_longest_period = get_actor_dead_for_the_longest_period(
            actor, actor_dead_for_the_longest_period
        )
        count_dead_actors = get_count_dead_actors(actor, count_dead_actors)
        count_alive_actors_age = get_average_age_alive_actors(
            actor, count_alive_actors_age
        )
        actors_with_same_dob = get_actors_with_same_dob(
            actor, actors, actors_with_same_dob
        )
...
def get_oldest_movie(actor, current_year, oldest_movie, oldest_movie_age):
    for movie in actor.movies:
        if (
            movie["year"] is not None
            and current_year - int(movie["year"]) > oldest_movie_age
        ):
...
def get_actor_dead_for_the_longest_period(
    actor, actor_dead_for_the_longest_period
) -> Actor:
    if actor.dod is not None:
        if get_period_of_death(actor) > get_period_of_death(
            actor_dead_for_the_longest_period
        ):
            return actor
...
def send_answers(answers):
    response = requests.post(
        "https://synapsi.xyz/api/recruitment/answers",
        data={
            "first_name": "Yaroslav",
            "last_name": "Sheshko",
            "email": "gharefda@gmail.com",
            "answers": get_all_answers(get_all_actors()),
        },
...
if __name__ == "__main__":
    print(send_answers(get_all_answers(get_all_actors())))
